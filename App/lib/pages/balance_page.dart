                      ...
                      : () async {
                          ...
                          try {
                            await _ble.sendCommand(
                              jsonEncode({'cmd': 'calib', 'weight': val}),
                            );
                            if (ctx.mounted) Navigator.pop(ctx);
                            Get.snackbar(
                              'Calibration lancée',
        ...
        builder: (ctx, setModalState) => Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              Text(
                'Le nouveau nom sera pris en compte au prochain redémarrage.',
                style: TextStyle(
                  fontSize: 14,
                  color: isDark ? Colors.grey[400] : Colors.grey[600],
                ),
                ...
                child: ElevatedButton.icon(
                  onPressed: isLoading
                      ...
                      : () async {
                          ...
                          try {
                            await _ble.sendCommand(
                              jsonEncode({'cmd': 'rename', 'name': name}),
                            );
                            _ble.deviceName.value = name;
                            if (ctx.mounted) Navigator.pop(ctx);
                            Get.snackbar(
                              'Renommée',
                              'Balance renommée en "$name".',
                              snackPosition: SnackPosition.BOTTOM,
                              backgroundColor: Colors.green[100],
                              colorText: Colors.green[900],
                            );
